...
cp -f ~/overcloud-deploy/overcloud/config-download/overcloud/tripleo-ansible-inventory.yaml ~/inventories/tripleo-ansible-inventory-overcloud.yaml
cp -f ./overcloud-deploy/cell1/config-download/cell1/tripleo-ansible-inventory.yaml ~/inventories/tripleo-ansible-inventory-cell1.yaml

cell_crlr_name=$(ansible-inventory -i ~/inventories/tripleo-ansible-inventory-cell1.yaml --list | jq -r '.CellController|.hosts[0]|select(. != null)')
CELL_CTRL_IP=$(ansible-inventory -i ~/inventories/tripleo-ansible-inventory-cell1.yaml --host $cell_crlr_name | jq -r .ctlplane_ip)
CTRL_IP=$(ansible-inventory -i ~/inventories/tripleo-ansible-inventory-overcloud.yaml --host controller-0 | jq -r .ctlplane_ip)
CELL_INTERNALAPI_INFO=$(ssh heat-admin@${CTRL_IP} egrep cell1.internalapi /etc/hosts)
